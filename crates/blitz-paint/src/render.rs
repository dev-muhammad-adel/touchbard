...
impl ElementCx<'_> {
    ...
    fn draw_border_edge(&self, sb: &mut impl PaintScene, edge: Edge) {
        ...
        // A zero-width border edge contributes no visible area (CSS draws
        // nothing for `border-width: 0` regardless of style or color). Skip it:
        // with a non-zero border-radius the corner geometry for a zero-width
        // border degenerates (0/0 → NaN) and would emit an invalid path.
