//! `/showcase/tiles` — the original capabilities playground, preserved.
//!
//! Before the current home page became the compact "Touch Bar Playground", this
//! tile grid was the home screen (`app/page.rs`). It is kept as its own route
//! so nothing was lost: every tile is a *live* sample of one capability of the
//! current rendering stack and jumps to the showcasing page it stands for on
//! tap. Only things the stack has actually been proven to do (see the
//! `showcase/*` pages and the CSS-animation regression test in
//! `touchbard/tests`) are used here: no icons, no `<svg>`, no `onpointer*`, no
//! invented APIs. The pulsing dot below is a real CSS animation driven by the
//! render loop.
